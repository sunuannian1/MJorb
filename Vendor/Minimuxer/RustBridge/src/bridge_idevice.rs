...
use crate::idevice_support::core_device_install::{
    install_via_core_tunnel, stage_and_install_via_core_tunnel, stage_via_core_tunnel,
};
...
#[no_mangle]
pub extern "C" fn rust_bridge_idevice_stage_and_install_via_core_tunnel(
    bundle_id: *const c_char,
    ipa_ptr: *const u8,
    ipa_len: u32,
) -> *mut IdeviceFfiError {
    ...
    runtime.block_on(async move {
        match stage_and_install_via_core_tunnel(bundle_id, ipa_bytes).await {
            Ok(()) => std::ptr::null_mut(),
            Err(err) => crate::ffi_err!(err),
        }
    })
...
#[no_mangle]
pub extern "C" fn rust_bridge_idevice_stage_via_core_tunnel(
    bundle_id: *const c_char,
    ipa_ptr: *const u8,
    ipa_len: u32,
) -> *mut IdeviceFfiError {
    ...
    runtime.block_on(async move {
        match stage_via_core_tunnel(bundle_id, ipa_bytes).await {
            Ok(()) => std::ptr::null_mut(),
            Err(err) => crate::ffi_err!(err),
        }
    })
...
#[no_mangle]
pub extern "C" fn rust_bridge_idevice_install_via_core_tunnel(
    bundle_id: *const c_char,
) -> *mut IdeviceFfiError {
    let Some(bundle_id) = c_string_arg(bundle_id) else {
        return invalid_argument_error();
    ...
    runtime.block_on(async move {
        match install_via_core_tunnel(bundle_id).await {
            Ok(()) => std::ptr::null_mut(),
            Err(err) => crate::ffi_err!(err),
        }
    })
